...
export default function Showcase () {
    ...
    const addToBarRef = el => {
        if (el && !barRef.current.includes(el)) {
            barRef.current.push(el)
        }
    }
    ...
    const Project = ({data}) => {
        ...
        return (
            <div onClick={() => window.open("https://"+data.url)} ref={addToDivRef} className={style.project}>
                <div ref={addToBarRef} className={style.bar}/>
                <span className={style.cat} ><p className='label'>{data.type}</p></span>
                <img src="/assets/icon/arrow.svg"/>
                <h3 ref={addToTitleRef}>{splitText.map((el, i) => <span className='animateSpanTitle' key={data.name+i}>{el}</span>)}</h3>
            </div>
        )
    }
    ...
    useEffect(() => {
        ...
        const scrollDivs = gsap.utils.toArray(divRef.current);
        console.log(scrollDivs);

          scrollDivs.forEach((div, i) => {
            const title = div.getElementsByClassName('animateSpanTitle')
            const bar = div.getElementsByTagName('div')[0]
            console.log(bar);
